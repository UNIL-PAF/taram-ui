...
import {setStepParameters, updatePlotOptions} from "../BackendAnalysisSteps";
...
export default function BoxPlot(props) {
    ...
    if(selCol){
        const echartsHash = b_crc32(JSON.stringify(options))

        if(!results.plot || results.plot.echartsHash !== echartsHash){
            console.log("update params for " + props.data.id)
            const plotParams = {echartsOptions: JSON.stringify(options), echartsHash: echartsHash}
            dispatch(updatePlotOptions({stepId: props.data.id, params: plotParams}))
        }
