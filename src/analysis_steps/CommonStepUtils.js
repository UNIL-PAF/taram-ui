import {updatePlotOptions} from "./BackendAnalysisSteps";
import React from "react";
import {Avatar, Button, Col, Modal, Row, Tag} from 'antd';
...
export function computeCrC32(str) {
    let crc = -1;
...
export function replacePlotIfChanged(stepId, results, options, dispatch){
    ...
    if(!results.plot || results.plot.echartsHash !== echartsHash){
        const plotParams = {echartsOptions: JSON.stringify(options, function(key, value) {
                if (typeof value === "function") {
                    return "/Function(" + value.toString() + ")/";
                }
                return value;
            }), echartsHash: echartsHash}
        dispatch(updatePlotOptions({stepId: stepId, params: plotParams}))
    }
}

export function getStepTitle(nr, type){
    return <div>
...
export const getTableCol = (nrProteinGroups, tableNr, setShowTable) => {
    return <Col span={8}>
            <Row className={"analysis-step-row-end"}>
                <span>{nrProteinGroups} protein groups</span>
            </Row>
        {tableNr &&
